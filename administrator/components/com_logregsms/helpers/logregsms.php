...
class LRSHelper
{
	...
	function __construct() {
		...
		// for router Instance
		if(empty(self::$_JApplication)) {
			self::$_JApplication    = JApplication::getInstance('site');
		}
     ...
     */
    public static function addSubmenu($viewName = 'smsarchives') {
	...
	*/
	...
	public static function RegisterUser($name, $email, $username, $mobile) {
		...
		if(!empty($check)) {
			JError::raiseWarning( 100, "نام کاربری قبلا انتخاب شده است. لطفا یک نام کاربری دیگری انتخاب کنید.");
			return null;
		}
		...
		if(!empty($check)) {
			JError::raiseWarning( 100, "ایمیل انتخاب شده از قبل درون سامانه استفاده شده است. لطفا یک آدرس ایمیل دیگری را وارد کنید." );
			return null;
		}
	...
	public static function Validation ($field, $type) {
		...
		switch ($type) {
			...
			case "mobile" : // Validate Mobile number
				...
				if(($split[0] !== "0" || $split[1] !== "9") && ($split[0] !== "9")) {
					$result['result'] = false;
					$result['msg'] = "شماره تلفن همراه باید با 09 شروع شود.";
					break;
				}
		
				if(count($split) !== 11 && count($split) !== 10) {
					$result['result'] = false;
