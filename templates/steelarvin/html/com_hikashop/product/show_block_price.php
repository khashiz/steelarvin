		...
		foreach($this->row->prices as $k => $price) {
			...
			if(!empty($this->row->discount)) {
				if(in_array($this->params->get('show_discount'), array(1, 4))) {
					echo '<div class="uk-text-zero uk-margin-small-bottom"><span class="hikashop_product_discount fullRound uk-label uk-label-success uk-text-tiny uk-text-bold uk-border-rounded">'.JText::_('PRICE_DISCOUNT_START');
