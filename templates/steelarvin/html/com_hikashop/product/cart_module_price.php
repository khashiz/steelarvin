...
if(!empty($show_msrp)) {
?>
	<span class="hikashop_product_msrp_price hikashop_product_price_full uk-display-block">
		<span class="hikashop_product_msrp_price_title uk-display-block"><?php
			echo JText::_('PRODUCT_MSRP_BEFORE');
		?></span>
		<span class="hikashop_product_price uk-display-block"><?php
			$mainCurr = $this->currencyHelper->mainCurrency();
			$currCurrency = hikashop_getCurrency();
...
?>
	<span class="hikashop_product_price uk-text-tiny uk-text-accent uk-display-block font <?php echo $class; ?>">
        <?php
	...
	if(empty($this->row->prices)) {
		...
		foreach($this->row->prices as $k => $price) {
			...
			if(!empty($this->row->discount)) {
				$classes[]='hikashop_product_price_with_discount uk-text-success';
			}

			if(!empty($this->row->discount)) {
				if(in_array($this->params->get('show_discount'), array(1, 4))) {
					echo '<div class="uk-text-zero uk-margin-small-bottom uk-hidden"><span class="hikashop_product_discount fullRound uk-label uk-label-success uk-text-tiny uk-text-bold uk-border-rounded">'.JText::_('PRICE_DISCOUNT_START');
				...
				if(in_array($this->params->get('show_discount'), array(2, 4))) {
					echo '<span class="hikashop_product_price_before_discount uk-text-tiny uk-text-linethrough">'.JText::_('PRICE_DISCOUNT_START');
