...
<?php
	if($this->image->override) {
		...
		if(@$img->success) {
			...
			$attr = 'class="uk-display-block" title="'.$this->escape(@$image->file_description).'"';
...
?>
	...
	<div id="hikashop_small_image_div<?php echo $variant_name;?>" class="uk-child-width-1-3 uk-child-width-1-4@m uk-grid-small" data-uk-grid data-uk-lightbox="animation: slide">
        <?php if( !empty($this->element->images) && count($this->element->images) > 1) {
            $firstThunb = true;
            foreach($this->element->images as $image) {
                ...
                $attr = 'class="uk-display-block uk-border-rounded uk-overflow-hidden uk-box-shadow-small" title="'.$this->escape(@$image->file_description).'" onmouseover="return window.localPage.changeImage(this, \'hikashop_main_image'.$variant_name.'\', \''.$img->url.'\', '.$img->width.', '.$img->height.', \''.str_replace(array("'", '"'),array("\'", '&quot;'),@$image->file_description).'\', \''.str_replace(array("'", '"'),array("\'", '&quot;'),@$image->file_name).'\');"';
                $html = '<img class="'.$classname.'" title="'.$this->escape(@$image->file_description).'" alt="'.$this->escape(@$image->file_name).'" src="'.$img->url.'"/>';
                if($this->config->get('add_webp_images', 1) && function_exists('imagewebp') && !empty($img->webpurl)) {
                    $html = '<picture><source srcset="'.$img->webpurl.'" type="image/webp"><source srcset="'.$img->url.'" type="image/'.$img->ext.'">'.$html.'</picture>';
                }
