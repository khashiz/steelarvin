...
class plgSystemMinitekSystemMessages extends JPlugin
{
	...
	{
		if (
			($this->app->isAdmin() && $this->params->get('enable_backend', false)) ||
			($this->app->isAdmin() && $this->params->get('enable_validation_backend', false)) ||
			($this->app->isSite() && $this->params->get('enable_frontend', true)) ||
			($this->app->isSite() && $this->params->get('enable_validation_frontend', true))
		)
		{
			$this->loadAssets();
		}
	...
	/**
	 * After Render Event.
	 * Add hidden class to system-message-container.
	 *
	 * @return   void
	 *
	 * @since   3.0.2
	...
	public function onAfterRender()
	{
		...
		if (
			($this->app->isAdmin() && $this->params->get('enable_backend', false)) ||
			($this->app->isSite() && $this->params->get('enable_frontend', false))
		)
		{
			// Get the body
			...
			$html = preg_replace('#<div id="'.$joomla_container.'">#', '<div id="'.$joomla_container.'" class="msm-hidden">', $html);
	...
	public function loadAssets()
	{
		...
		$is_site = $this->app->isSite() ? 'true' : 'false';
		...
		// Load css
		$document->addStyleSheet(JURI::root(true).'/plugins/system/miniteksystemmessages/assets/css/style.css');
		...
		// Load mGrowl
		$document->addStyleSheet(JURI::root(true).'/plugins/system/miniteksystemmessages/assets/css/jquery.mgrowl.css');
		$document->addScript(JURI::root(true).'/plugins/system/miniteksystemmessages/assets/js/jquery.mgrowl.js');

		// Add javascript variables
		$document->addScriptDeclaration('window.miniteksystemmessages = {
			lifetime: '.JFactory::getConfig()->get('lifetime').',
			user_id: "'.$this->user->id.'",
			close_all: "'.JText::_('PLG_SYSTEM_MINITEK_SYSTEM_MESSAGES_CLOSE_ALL_TEXT').'",
			mg_pool: "'.$pool.'",
			mg_closer: "'.$closer.'",
			mg_sticky: "'.$sticky.'",
			mg_theme: "'.$theme.'",
			...
			header_title: "'.JText::_('PLG_SYSTEM_MINITEK_SYSTEM_MESSAGES_HEADER_TITLE').'",
		};');

		// Load js
		$document->addScript(JURI::root(true).'/plugins/system/miniteksystemmessages/assets/js/script.js');
	}
}
